...
/** Loads audiofiles for playback using libsndfile and rtAudio.
 **/

class AudioPlayerRTA : apf::NonCopyable
{
  public:
    using ptr_t = std::unique_ptr<AudioPlayerRTA>; ///< unique_ptr to AudioPlayer
...
class AudioPlayerRTA::Soundfile : apf::NonCopyable
{
  public:
    ...
    Soundfile(const std::string& filename, bool loop) throw (soundfile_error);
