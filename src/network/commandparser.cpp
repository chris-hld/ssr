...
ssr::CommandParser::parse_cmd(const std::string& cmd)
{
  ...
  if (!doc1)
  {
    SSRERROR("Unable to load string! (\"" << cmd << "\")");
    return;
  }
  ...
  for (XMLParser::Node i = node1.child(); !!i; ++i)
  {
    if (i == "source")
    {
      ...
      if (!new_source)
      {
        ...
        {
          SSRERROR("No source ID specified!");
          return;
        }
      }
      ...
      if (id == "" && !new_source)
      {
        SSRERROR("Invalid source number: " << source_number);
        return;
      }
      ...
      {
        SSRERROR("Cannot set properties_file! This works only for new sources.");
      }
      ...
      {
        SSRERROR("Cannot set port name! This works only for new sources.");
      }
      ...
      {
        SSRERROR("Cannot set file name! This works only for new sources.");
      }
      if (!file_name.empty() && !port_name.empty())
      {
        SSRERROR("Either file name or port name can be set, not both!");
      }
      if (new_source && file_name.empty() && port_name.empty())
      {
        SSRERROR("Either file name or port name must be specified!");
      }
      ...
      if (S2A(i.get_attribute("channel"), channel))
      {
        ...
        if (file_name.empty())
        {
          SSRERROR("'channel' is only allowed if a file name is also specified!");
          channel = 0;
        }
        else
        {
    ...
    else if (i == "reference")
    {
      for (XMLParser::Node inner_loop = i.child(); !!inner_loop; ++inner_loop)
      {
        if (inner_loop == "position")
        {
          ...
          else SSRERROR("Invalid reference position!");
        }
        else if (inner_loop == "orientation")
        {
          float azimuth;
          ...
          else SSRERROR("Invalid reference orientation!");
        }
      }
    } // if (reference)
    else if (i == "reference_offset")
    {
      for (XMLParser::Node inner_loop = i.child(); !!inner_loop; ++inner_loop)
      {
        if (inner_loop == "position")
        {
          ...
          else SSRERROR("Invalid reference offset position!");
        }
        else if (inner_loop == "orientation")
        {
          float azimuth;
          ...
          else SSRERROR("Invalid reference offset orientation!");
        }
      }
    } // if (reference_offset)
    else if (i == "delete")
      ...
      for (XMLParser::Node inner_loop = i.child(); !!inner_loop; ++inner_loop)
      {
        if (inner_loop == "source")
        {
          ...
          if (S2A(inner_loop.get_attribute("id"), source_number))
          {
            ...
            {
              SSRERROR("Source ID doesn't exist: " << id);
            }
          }
          else SSRERROR("Cannot read source ID!");
        }
        else {}
      }// for ()
    }// else if (delete)
    else if (i== "scene")
    {
      ...
      else SSRERROR("Invalid Volume Setting! (\"" << volume_str << "\")");
    ...
    else if (i == "state")
    {
      ...
      {
        SSRERROR("Invalid value for \"processing\": " << processing);
      }
      ...
      {
        SSRERROR("Invalid value for \"transport\": " << transport);
      }
      ...
      {
        SSRERROR("Couldn't get the time out of the \"seek\" attribute (\""
            << seek << "\").");
      }
      ...
      {
        SSRERROR("Invalid value for \"tracker\": " << tracker);
      }
    }
  }
}
